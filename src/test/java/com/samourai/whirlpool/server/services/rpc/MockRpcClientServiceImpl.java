...
public class MockRpcClientServiceImpl implements RpcClientService {
    ...
    @Override
    public Optional<RpcRawTransactionResponse> getRawTransaction(String txid) throws GenericRpcException {
        // load mock from map
        ...
        RpcRawTransactionResponse rpcTxResponse = new RpcRawTransactionResponse(rpcTxHex.get(), MOCK_TX_CONFIRMATIONS);
        return Optional.of(rpcTxResponse);
    }

    public void mock(String txid, String rawTxHex, int confirmations) {
        log.info("mock tx: " + txid);
        RpcRawTransactionResponse rawTxResponse = new RpcRawTransactionResponse(rawTxHex, confirmations);
    ...
    public TxOutPoint createAndMockTxOutPoint(SegwitAddress address, long amount, Integer nbConfirmations, String utxoHash, Integer utxoIndex) throws Exception{
        ...
        // add coinbase input
        int txCounter = 1;
        TransactionInput transactionInput = new TransactionInput(params, transaction, new byte[]{(byte) txCounter, (byte) (txCounter++ >> 8)});

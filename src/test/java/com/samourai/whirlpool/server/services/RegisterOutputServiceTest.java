...
public class RegisterOutputServiceTest extends AbstractIntegrationTest {
    ...
    private void registerInput(Mix mix, String username) throws Exception {
        ...
        TxOutPoint txOutPoint = rpcClientService.createAndMockTxOutPoint(new SegwitAddress(pubkey, cryptoService.getNetworkParameters()), inputBalance, 999);
    ...
    private byte[] confirmInput(Mix mix, String username, String receiveAddress, RSABlindingParameters blindingParams) throws Exception {
        ...
        byte[] signedBlindedBordereau = cryptoService.signBlindedOutput(blindedBordereau, mix.getKeyPair());
    ...
    public void registerOutput_shouldSuccessWhenValid() throws Exception {
        ...
        registerOutputService.registerOutput(mix.computeInputsHash(), unblindedSignedBordereau, receiveAddress);
    ...
    public void registerOutput_shouldFailWhenBlindedBordereauFromInvalidPubkey() throws Exception {
        ...
        RSABlindingParameters blindingParams = clientCryptoService.computeBlindingParams(fakePublicKey); // blind from INVALID pubkey
    ...
    public void registerOutput_shouldFailWhenBordereauFromPreviousMix() throws Exception {
        ...
        byte[] signedBlindedBordereauFirstMix = confirmInput(mix, username, receiveAddress, blindingParams);
        ...
        byte[] signedBlindedBordereauSecondMix = confirmInput(mix, username, receiveAddress, blindingParams);
        ...
        byte[] unblindedSignedBordereauFirstMix = clientCryptoService.unblind(signedBlindedBordereauFirstMix, blindingParams);
        try {
            registerOutputService.registerOutput(mix.computeInputsHash(), unblindedSignedBordereauFirstMix, receiveAddress);
            Assert.assertTrue(false); // should throw exception
        } catch(Exception e) {
            // exception expected
            Assert.assertEquals("Invalid unblindedSignedBordereau", e.getMessage());
        ...
        byte[] unblindedSignedBordereauSecondMix = clientCryptoService.unblind(signedBlindedBordereauSecondMix, blindingParams);
        registerOutputService.registerOutput(mix.computeInputsHash(), unblindedSignedBordereauSecondMix, receiveAddress);
    ...
    public void registerOutput_shouldSuccessWhenInvalid() throws Exception {
        ...
        try {
            registerOutputService.registerOutput("invalidInputsHash", unblindedSignedBordereau, receiveAddress); // INVALID inputsHash
        ...
        try {
            ...
            registerOutputService.registerOutput(mix.computeInputsHash(), fakeSignedBordereau, receiveAddress);
            Assert.assertTrue(false);
        } catch(Exception e) {
            Assert.assertEquals("Invalid unblindedSignedBordereau", e.getMessage());
        }
        ...
        try {
            registerOutputService.registerOutput(mix.computeInputsHash(), unblindedSignedBordereau, "tb1qnfhmn4vfgprfnsnz2fadfr48cquydjkz4wpfgq"); // INVALID receiveAddress
            Assert.assertTrue(false);
        } catch(Exception e) {
            Assert.assertEquals("Invalid unblindedSignedBordereau", e.getMessage());
        }

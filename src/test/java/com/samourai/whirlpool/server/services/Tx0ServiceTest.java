...
public class Tx0ServiceTest extends AbstractIntegrationTest {
  ...
  public void checkInput_useCache() throws Exception {
    ...
    // VERIFY
    Mockito.verify(tx0ServiceSpy, Mockito.times(1))
        .doCheckInputCacheable(
            Mockito.any(RpcTransaction.class), Mockito.anyLong(), Mockito.anyList());

    // TEST: second call => cached
    tx0ServiceSpy = Mockito.spy(tx0Service);
    Assert.assertFalse(tx0ServiceSpy.checkInput(rpcOutWithTx));
    // VERIFY
    Mockito.verify(tx0ServiceSpy, Mockito.times(0))
        .doCheckInputCacheable(
            Mockito.any(RpcTransaction.class), Mockito.anyLong(), Mockito.anyList());
    ...
    rpcOutWithTx =
        blockchainDataService
            .getRpcOutWithTx(
                "7ea75da574ebabf8d17979615b059ab53aae3011926426204e730d164a0d0f16", utxoIndex)
            .orElseThrow(() -> new NoSuchElementException(utxoHash + "-" + utxoIndex));
    tx0ServiceSpy = Mockito.spy(tx0Service);
    try {
      ...
      // ok
      Assert.assertEquals(
          "7ea75da574ebabf8d17979615b059ab53aae3011926426204e730d164a0d0f16 is not a valid whirlpool tx, inputs/outputs count mismatch (verified path:mixInput:7ea75da574ebabf8d17979615b059ab53aae3011926426204e730d164a0d0f16-0)",
          e.getMessage());
    }
    // VERIFY
    Mockito.verify(tx0ServiceSpy, Mockito.times(1))
  ...
  public void checkWhirlpoolTx_invalidMix() throws Exception {
    ...
    thrown.expectMessage(
        "781bc448f381e160d014f5515daf8a175bb689112c7755c70c6bbdbb32af56cc is not a valid whirlpool tx, inputs/outputs count mismatch (verified path:mixInput:781bc448f381e160d014f5515daf8a175bb689112c7755c70c6bbdbb32af56cc-5)");
    doCheckWhirlpoolTx(txid, 10000000);
  }
  ...
  private boolean doCheckInput(String utxoHash, long utxoIndex, long minFees)

...
public class PoolService {
    ...
    @Autowired
    public PoolService(WhirlpoolServerConfig whirlpoolServerConfig, CryptoService cryptoService, WebSocketService webSocketService) {
    ...
    public SubscribePoolResponse computeSubscribePoolResponse(String poolId) throws MixException {
        ...
        SubscribePoolResponse poolStatusNotification = new SubscribePoolResponse(cryptoService.getNetworkParameters().getPaymentProtocolId(), pool.getDenomination(), pool.getMinerFeeMin(), pool.getMinerFeeMax());
        return poolStatusNotification;
    }

    public synchronized void registerInput(String poolId, String username, byte[] pubkey, boolean liquidity, TxOutPoint input) throws IllegalInputException, MixException {
        Pool pool = getPool(poolId);
        ...
        if (!isUtxoConfirmed(input, liquidity)) {
            log.info(" • [" + poolId + "] queued UNCONFIRMED UTXO " + (liquidity ? "liquidity" : "mustMix") + ": " + input);
    ...
    private void queueToPool(Pool pool, RegisteredInput registeredInput) {
        ...
        log.info(" • [" + pool.getPoolId() + "] queued " + (registeredInput.isLiquidity() ? "liquidity" : "mustMix") + ": " + registeredInput.getInput());
    }
    ...
    private void inviteToMix(Mix mix, RegisteredInput registeredInput) {
        ...
        ConfirmInputMixStatusNotification confirmInputMixStatusNotification = new ConfirmInputMixStatusNotification(mix.getMixId(), publicKey64);
        mix.registerConfirmingInput(registeredInput);

        log.info(" • [" + mix.getMixId() + "] inviting " + (registeredInput.isLiquidity() ? "liquidity" : "mustMix") + " to mix: " + registeredInput.getInput());
    ...
    public synchronized int inviteAllToMix(Mix mix, boolean liquidity) {
        InputPool queue = (liquidity ? mix.getPool().getLiquidityQueue() : mix.getPool().getMustMixQueue());
    ...
    private boolean isUtxoConfirmed(TxOutPoint txOutPoint, boolean liquidity) {
        ...
        if (liquidity) {
            ...
            if (inputConfirmations < minConfirmationsMix) {
                log.info("input not confirmed: liquidity needs at least " + minConfirmationsMix + " confirmations: " + txOutPoint.getHash());
                return false;
            }
        } else {
            // mustMix
            ...
            if (inputConfirmations < minConfirmationsTx0) {
                log.info("input not confirmed: mustMix needs at least " + minConfirmationsTx0 + " confirmations: " + txOutPoint.getHash());
                return false;
            }
        }
        return true;
    ...
    public synchronized void onClientDisconnect(String username) {
        for (Pool pool : getPools()) {
            ...
            if (liquidityRemoved) {
                log.info(" • [" + pool.getPoolId() + "] removed 1 liquidity from pool, username=" + username);
            }
            ...
            boolean unconfirmedInputRemoved = pool.getUnconfirmedQueue().removeByUsername(username).isPresent();
            if (unconfirmedInputRemoved) {
                log.info(" • [" + pool.getPoolId() + "] removed 1 unconfirmed UTXO from pool, username=" + username);
            }
        }
    }
}
